...
class OrdersToSyncInExternalSystemAdminResource extends Lens
{
    ...
    public function fields(NovaRequest $request): array
    {
        ...
        return [
            ...
            Select::make('Способ оплаты', 'paymentMethod')
                ...
                ->hideWhenCreating()
                ->hideWhenUpdating(),
            Boolean::make('Синхронизировано', 'metadata->is_synced_in_external_system')
                ->sortable()
                ->hideWhenCreating()
                ->hideWhenUpdating()
                ->displayUsing(function ($isSynced) {
                    return $isSynced ? 'Да' : 'Нет';
                }),
        ];
    }
    ...
    {
        // where not deleted and where metadata->is_synced_in_external_system is false

        $query = $query->where('deleted_at', '=', null)
            ->whereIn('status', [OrderStatus::CONFIRMED])
            ->whereJsonContains('metadata->is_synced_in_external_system', false);
