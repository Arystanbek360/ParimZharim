...
class LogoutCurrentDevice extends BaseAction {
    ...
    public function handle(User $user): void {
        DB::beginTransaction();
        try {
            $this->personalAccessTokenRepository->deleteCurrentUserToken($user);
            $this->userDeviceRepository->deleteCurrentUserDevice($user);
            DB::commit();
        }
        catch (Throwable $e) {
            DB::rollBack();
