...
class CloudPaymentService extends BaseService implements CloudPaymentServiceInterface
{
    ...
    public function __construct(
        ...
        public string                      $base_url = "https://api.cloudpayments.kz/")
    {
    ...
    public function getPaymentData(Payment $payment): array
    {
        return [
            ...
            'accountId' => $payment->customer_id, // Уникальный ID пользователя или заказа
        ];
    }

    public function getPaymentForm(Payment $payment): ?string
    {
        return 'payment::components.cloudpayments-form';
    }
    ...
    public function completePayment(int $paymentID): void
    {
        ...
        if ($payment->status === PaymentStatus::SUCCESS) {
            $response = $this->auth()->post($this->base_url . 'payments/confirm', ['TransactionId' => $payment->external_id, 'Amount' => $payment->total]);
            $data = $response->json();
            if ($data['Success']) {
    ...
    {
        $response = $this->auth()->post($this->base_url . 'v2/payments/find', ['InvoiceID' => $paymentID]);
        $data = $response->json();
        ...
        if (isset($data['Model'])) {
            ...
            switch ($payment->status) {
                ...
                case PaymentStatus::FAILED:
                    PaymentFailed::dispatch($payment, $errorReason);
                    break;
                default:
                    break;
            }
    ...
    {
        return match ($status) {
            'AwaitingAuthentication'  => PaymentStatus::PENDING, // '3DSecure'
